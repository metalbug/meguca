...
// TODO: Asynchronously queue body writes to one second batches
// TODO: Ensure closed post body writes are synchronous and asynchronous open
// body writes do not override them. (use recently closed post ID map)
...
impl Feed {
	...
	fn diff_open_bodies(&mut self) {
		...
		{
			self.data.open_posts.get_mut(&id).unwrap().body = new;
